...
class UserModel {
    ...
    public static function getStats() {
        ...
        $stats['total_admins'] = db_fetch_one("SELECT COUNT(*) as count FROM users WHERE role = 'admin'")['count'];
    ...
    public static function updateProfile($id, $name, $phone, $department, $bio, $profile_pic = null) {
        if ($profile_pic) {
            $sql = "UPDATE users SET name = ?, phone = ?, department = ?, bio = ?, profile_pic = ? WHERE id = ?";
            return db_query($sql, [$name, $phone, $department, $bio, $profile_pic, $id], "sssssi");
        } else {
            $sql = "UPDATE users SET name = ?, phone = ?, department = ?, bio = ? WHERE id = ?";
